...
final class HealthKitService: HealthDataProviding {
    ...
    func queryRecentRecords(since startDate: Date, perMetricLimit: Int) async throws -> [HealthRecord] {
        ...
        for metric in HealthMetric.allCases {
            ...
            records.append(contentsOf: samples.compactMap { makeRecord(from: $0, metric: metric) })
        }

        return records.sorted { $0.startDate > $1.startDate }
    }

    private func sampleType(for metric: HealthMetric) -> HKSampleType? {
        switch metric {
        case .steps:
    ...
    private func makeRecord(from sample: HKSample, metric: HealthMetric) -> HealthRecord? {
        var value: Double
        var unit: String
