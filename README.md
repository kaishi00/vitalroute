...
The app has no analytics, advertising, accounts, or vendor-cloud integration. Health records are held in memory only for the operation you started — there is no local outbox or health database on the device. The destination endpoint and its API key live in the Keychain. Automatic sync keeps captured health changes in a data-protected outbox under Application Support until the receiver acknowledges them; sync metadata (checkpoints, retry bookkeeping, and the identity of the destination the queue belongs to) is stored alongside, and nothing is excluded from the device's own data protection. The destination URL must be HTTPS without credentials, query strings, or fragments; requests follow no redirects.
