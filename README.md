...
- **Explicit export selection** — the categories (steps, heart rate, resting heart rate, heart rate variability, sleep, active energy, workouts) are opt-in toggles; nothing is selected by default, and VitalRoute requests HealthKit read access only for the categories you enable. The record model is a typed envelope that already represents the broader HealthKit families (categories, correlations, workouts, activity summaries, ECGs and series data, clinical FHIR) — see `docs/RECORD-ARCHITECTURE.md` — so growing the catalog is descriptor work, not another redesign.
...
- **Manual sync** — "Sync Now" reads the configured history window (7 days by default; configurable up to all records) for the selected categories (not the dashboard's 20-sample preview), uploads in bounded batches over HTTPS with normal certificate validation and no redirects, and reports acknowledged counts, partial failures, and cancellation honestly. Reading is resumable: each category advances in anchored 500-record chunks whose cursor is saved only after acknowledgement, so a large historical backfill continues across successive syncs instead of failing on an oversized read; deepening the configured history backfills the older data over those continued syncs. Retrying is safe: the receiver stores each record once.
...
- **Reference receiver** — `server/` contains a dependency-free Python receiver with Bearer-token auth, strict envelope validation with no metric allowlist, transactional SQLite persistence, idempotent ingestion, and (contract v3) typed record payloads plus deletion events with tombstones. See `server/README.md`, the contract in `server/API.md`, and the background-sync design in `docs/BACKGROUND-SYNC-DESIGN.md`.

Manual sync never runs on its own: saving configuration, opening the app, or refreshing the dashboard never uploads data. Automatic sync only works after an explicit opt-in, and only against a receiver that advertises deletion support (contract v3).
